    ...
    def validate_project(self) -> None:
        ...
        for name, path in mandatory_paths.items():
            if path is None or not Path(path).exists():
                raise Exception(f"Could not find {name} at {path} the value of project_yml_path is {project_yml_path} the value of mandatory_paths is {mandatory_paths}")
                #raise CosmosValueError(f"Could not find {name} at {path} the value of project_yml_path is {project_yml_path} the value of mandatory_paths is {mandatory_paths}")
